...
const Hero = () => {
  ...
  return (
    <section className="relative min-h-screen overflow-hidden bg-midnight">
      ...
      />
      <motion.div
        className="absolute left-1/2 top-10 h-32 w-[520px] -translate-x-1/2 rounded-full bg-neon/20 blur-[120px]"
        animate={{ opacity: [0.2, 0.5, 0.2], scale: [0.9, 1.05, 0.9] }}
        transition={{ duration: 8, repeat: Infinity, ease: 'easeInOut' }}
      />
      <motion.div
        className="absolute bottom-16 left-1/2 flex -translate-x-1/2 items-center gap-3 text-xs uppercase tracking-[0.4em] text-slate-400"
        initial={{ opacity: 0, y: 12 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 1.2, duration: 0.8 }}
      >
        <span className="h-px w-12 bg-neon/60" />
        Scroll
        <span className="h-px w-12 bg-neon/60" />
      </motion.div>
